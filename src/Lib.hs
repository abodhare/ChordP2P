...
listenHT :: HashTable B.ByteString B.ByteString -> (C.HostName, C.ServiceName) -> (C.HostName, C.ServiceName) -> IO ()
listenHT ht (ah, as) b = C.serve (C.Host ah) as $ \(connSoc, remoteAddr) -> do
...
runLoop :: HashTable B.ByteString B.ByteString -> (C.HostName, C.ServiceName) -> C.Socket -> IO ()
runLoop ht b connSoc = do
    s <- C.recv connSoc 128
    case s of
        Nothing -> C.send connSoc "not available"
        Just val -> case B.words val of
                        ["get", x] -> HT.lookup ht x >>= lookupOther b x >>= C.send connSoc >> runLoop ht b connSoc
                        ["getID", x] -> (lookupString <$> HT.lookup ht x) >>= C.send connSoc >> runLoop ht b connSoc
                        ["put", x, y] -> HT.insert ht x y >> C.send connSoc (insertString x y) >> runLoop ht b connSoc
                        ("q":_) -> C.send connSoc "shutting the server down, bye bye"
                        _          -> C.send connSoc helpString >> runLoop ht b connSoc

lookupOther :: (C.HostName, C.ServiceName) -> B.ByteString -> Maybe B.ByteString -> IO B.ByteString
...
lookupOther_ (bh, bs) x = C.connect bh bs $ \(connSoc, remoteAddr) -> do
